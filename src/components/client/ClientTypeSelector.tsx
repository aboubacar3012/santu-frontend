...
const ClientTypeSelector = ({ clientType, onChangeType }: ClientTypeSelectorProps) => {
  return (
    <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
      ...
      <div className="flex gap-4">
        ...
        >
          ...
          <span
            className={`flex h-5 w-5 items-center justify-center rounded-full border ${
              clientType === TypeEnum.PARTICULAR ? 'border-green-500' : 'border-gray-300'
            }`}
          >
            <span
              className={`h-2.5 w-2.5 rounded-full ${
                clientType === TypeEnum.PARTICULAR ? 'bg-green-500' : 'bg-transparent'
              }`}
            ></span>
          </span>
        ...
        >
          ...
          <span
            className={`flex h-5 w-5 items-center justify-center rounded-full border ${
              clientType === TypeEnum.PROFESSIONAL ? 'border-green-500' : 'border-gray-300'
            }`}
          >
            <span
              className={`h-2.5 w-2.5 rounded-full ${
                clientType === TypeEnum.PROFESSIONAL ? 'bg-green-500' : 'bg-transparent'
              }`}
            ></span>
          </span>
