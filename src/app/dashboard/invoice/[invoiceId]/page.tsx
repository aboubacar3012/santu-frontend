        ...
        <div className="w-5/6 print:w-full pr-4 flex justify-between items-center">
          ...
          {invoiceData.status === 'DRAFT' && (
            <div className="bg-white rounded-xl w-60 p-2 flex flex-col no-print">
              <button
                type="button"
                className="text-xs hover:text-white border border-green-700 hover:bg-green-800 text-green-800  font-medium rounded-lg px-3 py-2.5"
              >
                Marqué comme payé
              </button>
            </div>
          ...
          <div className="w-5/6 print:w-full flex flex-col gap-2 bg-white p-4 rounded-xl">
            <div className="w-full flex justify-between">
              <div className="flex gap-2">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm text-black">{loggedAccount?.address}</p>
                  {/* Tel */}
                  <p className="text-sm text-black">{loggedAccount?.phone}</p>
                  {/* Email */}
                  <p className="text-sm text-black">{loggedAccount?.email}</p>
                </div>
              </div>
              <div className="flex flex-col justify-between items-end gap-6">
                <div className="flex gap-4">
                  {invoiceData.status === 'DRAFT' && (
                    <Badge type="gray" text="Facture en brouillon" />
                  )}
                  {invoiceData.status === 'SENT' && (
                    <Badge type="blue" text="Facture envoyée au client" />
                  )}
                  {invoiceData.status === 'PAID' && (
                    <Badge type="green" text="Facture payée" />
                  )}
                  {invoiceData.status === 'CANCELLED' && (
                    <Badge type="red" text="Facture annulée" />
                  )}
                  <Badge type="gray" text={invoiceData.invoiceNumber} />
                </div>
                <div className="text-right">
                  <p className="font-light text-xs">Montant total</p>
                  <p className="font-bold text-lg">{invoiceData.amount} GNF</p>
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-black">
                    {invoiceData.paymentCondition === 'NOW'
                      ? 'Immédiat'
                      : `Dans ${invoiceData.paymentCondition} jours`}
                  </p>
                </div>
                <div>
                  <h2 className="text-xs font-light">Mode de paiement</h2>
                  <p className="text-xs font-semibold text-black">Espèces</p>
                </div>
              </div>
                ...
                <tbody className="">
                  {invoiceData?.articles.map((article, index) => (
                    ...
                    >
                      ...
                      <td className="text-xs px-6 py-2">{article.quantity}</td>
                      <td className="text-xs px-6 py-2">
            ...
            <div className="w-full flex justify-end mt-4 relative">
              <div className="absolute left-1/3 top-6 transform -translate-x-1/2">
                <Stamp type="unpaid" />{' '}
                {/* Updated the type for the Stamp component */}
              </div>
          ...
          <div className="w-1/6 flex flex-col gap-4 no-print">
            <div className="bg-white rounded-xl w-54 p-2 flex flex-col">
              <button
                ...
                className="text-white bg-gray-700 hover:bg-gray-800 font-medium rounded-lg text-sm py-2"
              >
                Imprimer la facture
              </button>
            </div>
            <div className="bg-white rounded-xl w-54 p-2 flex flex-col">
              ...
              <button
                onClick={() => {
                  alert(
                    'Fonctionnalité en cours de développement, elle sera bientôt disponible'
                  );
                }}
                type="button"
                className=" text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm  py-2"
              >
