...
const DashboardPage = () => {
  ...
  const handleDateFilterChange = (period: 'today' | 'week' | 'month' | 'year') => {
    ...
    setLoading(true);
    getDashboard(auth.loggedAccountInfos?._id!, auth.token!, period, selectedPaymentFilterBtn)
      .then(data => {
        if (data.success) {
          setDashboardData(data.dashboardData);
          setLoading(false);
  ...
  const handlePaymentFilterChange = (
    ...
    // Recharger les données avec le nouveau filtre
    setLoading(true);
    getDashboard(auth.loggedAccountInfos?._id!, auth.token!, selectedDateFilterBtn, status)
      .then(data => {
        if (data.success) {
          setDashboardData(data.dashboardData);
          setLoading(false);
  ...
  return (
    <div>
      ...
      <div className="relative flex flex-col text-black bg-white shadow-md w-full rounded-xl bg-clip-border">
        <div className="relative grid mx-4 mb-3 overflow-hidden text-white shadow-lg h-16 place-items-center rounded-xl bg-gradient-to-tr from-my-raspberry to-my-eggplant bg-clip-border shadow-my-raspberry-900/20">
            ...
            {dashboardData.invoices.map((invoice: Partial<Invoice>, index: number) => (
              ...
              >
                ...
                <td className="px-4 py-4 flex">
                  {invoice.status === 'DRAFT' && <Badge type="gray" text="Brouillon" />}
                  {invoice.status === 'SENT' && <Badge type="blue" text="Envoyée" />}
                  {invoice.status === 'PAID' && <Badge type="green" text="Déjà payée" />}
                  {invoice.status === 'CANCELLED' && <Badge type="red" text="Annulée" />}
                </td>
